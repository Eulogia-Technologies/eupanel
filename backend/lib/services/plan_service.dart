...
class PlanService {
  ...
  Future<Map<String, dynamic>> create(Map<String, dynamic> data) async {
    ...
    final plan = await Plan().create({
      ...
      'ftp_accounts_limit':
          _parseInt(data['ftp_accounts_limit'] ?? 1, 'ftp_accounts_limit'),
      'database_limit':
  ...
  Future<Map<String, dynamic>> update(
    ...
    if (data.containsKey('name'))
      updateData['name'] = data['name'].toString().trim();
    if (data.containsKey('description'))
      updateData['description'] = data['description'];
    if (data.containsKey('disk_limit'))
      updateData['disk_limit'] = _parseInt(data['disk_limit'], 'disk_limit');
    if (data.containsKey('bandwidth_limit'))
      updateData['bandwidth_limit'] =
          _parseInt(data['bandwidth_limit'], 'bandwidth_limit');
    if (data.containsKey('ftp_accounts_limit'))
      updateData['ftp_accounts_limit'] =
          _parseInt(data['ftp_accounts_limit'], 'ftp_accounts_limit');
    if (data.containsKey('database_limit'))
      updateData['database_limit'] =
          _parseInt(data['database_limit'], 'database_limit');
  ...
  void _validatePlanData(Map<String, dynamic> data) {
    ...
    if (data['disk_limit'] == null) {
      throw ValidationException({
        'disk_limit': ['Disk limit is required.']
      });
    }
    if (data['bandwidth_limit'] == null) {
      throw ValidationException({
        'bandwidth_limit': ['Bandwidth limit is required.']
      });
    }
...
// NotFoundException and ValidationException are provided by flint_dart
