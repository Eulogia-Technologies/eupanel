...
class WebsitesDomainsView extends FlintComponent {
  final String role;
  ...
  WebsitesDomainsView({
    ...
    _gitDeploys = ResourceController<List<FlintModelRecord>>(
      loader: () => FlintModelApi<FlintModelRecord>.records('/github/deploys').list(),
      loadImmediately: true,
    );
    _gitStatus = ResourceController<Map<String, dynamic>>(
      loader: () async {
        final response = await clientRouter.get<Map<String, dynamic>>('/github/status');
    ...
    _repos = ResourceController<List<Map<String, dynamic>>>(
      loader: () async {
        final response = await clientRouter.get<Map<String, dynamic>>('/github/repos');
        if (response.isError) return const [];
        final list = response.data?['data'];
        if (list is List) {
          return list.whereType<Map>().map((m) => m.map((k, v) => MapEntry(k.toString(), v))).toList();
        }
        return const [];
      },
  ...
  FlintNode build() {
    ...
    if (params['github'] == 'connected' && _gitStatus.data?['connected'] != true) {
      Future.microtask(() {
        _gitStatus.refresh();
      });
    }
    ...
    return EuPanelDashboardShell(
      ...
      topbar: Topbar(
        ...
        subtitle: 'Manage domains, paths, Let\'s Encrypt SSL, and Git deployments',
        dartStyle: const DartStyle(
          display: Display.flex,
          alignItems: AlignItems.center,
            ...
            Button(
              ...
              onPressed: (_) {
                ...
                if (subsList.isNotEmpty) {
                  _domainForm.setField('subscription_id', subsList.first.string('id') ?? '');
                }
        ...
        Grid(
          ...
          children: [
            Panel(
              ...
              description: 'Create and connect domains to your server environment.',
              dartStyle: const DartStyle(
                background: '#ffffff',
              ...
              child: ResourceView<List<FlintModelRecord>>(
                ...
                (snapshot) {
                  ...
                  if (list.isEmpty) {
                    return EmptyState(
                      title: snapshot.isError ? 'Could not load websites' : 'No websites hosted',
                  ...
                  final deploys = _gitDeploys.data ?? const <FlintModelRecord>[];
                  ...
                  return Column(
                    ...
                    children: [
                      ...
                      for (final domain in list)
                        _buildDomainCard(domain, deploys, subs),
                    ],
                  );
                },
              ),
      ...
      (snapshot) {
        ...
        if (connected) {
          return Container(
            dartStyle: const DartStyle(
              ...
              margin: EdgeInsets.only(bottom: 24),
              shadow: Shadow(x: 0, y: 4, blur: 12, color: Color.rgba(34, 197, 94, 0.05)),
            ),
            children: [
              Row(
                dartStyle: const DartStyle(
                ...
                children: [
                  Container(
                    dartStyle: const DartStyle(
                      ...
                      radius: 12,
                      background: '#ffffff',
                      shadow: Shadow(x: 0, y: 2, blur: 8, color: Color.rgba(0, 0, 0, 0.05)),
                    ),
                    child: Text.span('🚀', dartStyle: const DartStyle(fontSize: 20)),
                  ),
                  Column(
                    dartStyle: const DartStyle(
                      display: Display.flex,
                    ...
                    children: [
                      Text.strong(
                        ...
                        dartStyle: const DartStyle(fontSize: 15, fontWeight: 700, color: Color('#14532d')),
                      ),
                      Text.span(
                        'Successfully linked to @$username. Webhooks are configured.',
                        dartStyle: const DartStyle(fontSize: 13, color: Color('#166534')),
                      ),
                    ],
                  ),
                ],
        ...
        return Container(
          dartStyle: const DartStyle(
            ...
            margin: EdgeInsets.only(bottom: 24),
            shadow: Shadow(x: 0, y: 4, blur: 12, color: Color.rgba(14, 165, 233, 0.05)),
          ),
          children: [
            Row(
              dartStyle: const DartStyle(
              ...
              children: [
                ...
                Column(
                  ...
                  children: [
                    Text.strong(
                      'Link GitHub Account',
                      dartStyle: const DartStyle(fontSize: 15, fontWeight: 700, color: Color('#0369a1')),
                    ),
                    Text.span(
                      'Enable automated Git-to-Web deployments on your server in one click.',
                      dartStyle: const DartStyle(fontSize: 13, color: Color('#075985')),
                    ),
                  ],
                ),
              ],
            ...
            Link(
              ...
              dartStyle: const DartStyle(
                ...
                fontWeight: 700,
                cursor: Cursor.pointer,
                shadow: Shadow(x: 0, y: 4, blur: 12, color: Color.rgba(36, 41, 47, 0.2)),
              ),
  ...
  Future<void> _handleGithubDisconnect() async {
    ...
    final response = await clientRouter.delete<Map<String, dynamic>>('/github/disconnect');
    if (response.isError) {
  ...
  FlintNode _buildDomainCard(FlintModelRecord domain, List<FlintModelRecord> deploys, List<FlintModelRecord> subs) {
    ...
    final subName = sub != null ? (sub.string('system_username') ?? 'Subscription') : 'Subscription';
            ...
            Row(
              ...
              children: [
                Container(
                  ...
                  child: Text.span('🌐', dartStyle: const DartStyle(fontSize: 22)),
                ),
                Column(
                  dartStyle: const DartStyle(
                    display: Display.flex,
                ...
                Row(
                  ...
                  children: [
                    ...
                    Text.strong(
                      ...
                      dartStyle: const DartStyle(fontSize: 14, fontWeight: 700, color: Color('#334155')),
                    ),
                    if (hasDeploy)
                      Container(
                        dartStyle: const DartStyle(
                ...
                if (hasDeploy)
                  ...
                  Button(
                    ...
                    dartStyle: DartStyle(
                      ...
                      color: '#ffffff',
                      shadow: githubConnected ? const Shadow(x: 0, y: 2, blur: 6, color: Color.rgba(0, 0, 0, 0.1)) : null,
                    ),
                    onPressed: (_) {
                    ...
                    child: Text(githubConnected ? 'Link Repository' : 'Link Repository (Connect GitHub First)'),
                  ),
              ],
            ),
            if (hasDeploy)
              ...
              Text.span(
                ...
                dartStyle: const DartStyle(fontSize: 12, color: Color('#64748b'), lineHeight: 1.5),
              ),
          ],
        ),
          ...
          Select(
            ...
            options: [
              for (final sub in (subscriptions.data ?? []))
                SelectOption(
                  label: '${sub.string('system_username') ?? 'Subscription'} (${sub.string('id') ?? ''})',
                  value: sub.string('id') ?? '',
                ),
            ],
            onChanged: (event) {
          ...
          Row(
            ...
            children: [
              ...
              Button(
                ...
                child: Text(_domainForm.processing ? 'Provisioning...' : 'Add Website'),
              ),
            ],
          ),
        ],
            ...
            (snapshot) {
              ...
              if (reposList.isEmpty) {
                return EmptyState(
                  ...
                  message: snapshot.isError ? snapshot.error.toString() : 'Ensure your GitHub account has public or private repositories available.',
                );
              }

              if (_deployForm.string('repo_full_name').isEmpty && reposList.isNotEmpty) {
                _deployForm.setField('repo_full_name', reposList.first['full_name']?.toString() ?? '');
              }
          ...
          Row(
            ...
            children: [
              ...
              Button(
                ...
                child: Text(_deployForm.processing ? 'Linking...' : 'Link Repository'),
              ),
            ],
          ),
        ],
  ...
  Future<void> _handleDeleteDomain(String id, String name) async {
    final confirmed = web.window.confirm('Are you sure you want to remove the website $name? This will delete Nginx config files!');
    if (!confirmed) return;

    final response = await clientRouter.delete<Map<String, dynamic>>('/domains/$id');
    if (response.isError) {
      web.window.alert((response.error ?? 'Failed to delete domain.').toString());
      return;
    }
  ...
  Future<void> _handleDisconnectGit(String deployId) async {
    final confirmed = web.window.confirm('Disconnect GitHub repository deployment?');
    if (!confirmed) return;

    final response = await clientRouter.delete<Map<String, dynamic>>('/github/deploys/$deployId');
    if (response.isError) {
      web.window.alert((response.error ?? 'Failed to disconnect repository.').toString());
      return;
    }
